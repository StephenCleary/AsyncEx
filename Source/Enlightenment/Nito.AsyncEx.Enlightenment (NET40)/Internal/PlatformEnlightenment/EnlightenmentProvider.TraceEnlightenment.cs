...
namespace Nito.AsyncEx.Internal.PlatformEnlightenment
{
    ...
    {
        private sealed class TraceEnlightenment : ITraceEnlightenment
        {
            private static readonly TraceSource Trace = new TraceSource("Nito.AsyncEx");

            void ITraceEnlightenment.AsyncContext_TaskQueueFailed(AsyncContext context, Task task)
            {
                Trace.TraceEvent(TraceEventType.Error, 0, "Could not queue task " + task.Id + " to AsyncContext " + context.Id + " because the context has already completed.");
            }
